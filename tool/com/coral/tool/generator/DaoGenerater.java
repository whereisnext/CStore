 ...
 *         </p>
 */
public class DaoGenerater extends AbstractGenerator {
	...
	private String default_dao_path = "build\\dao";

	public DaoGenerater(Coral coral) {
		this.coral = coral;
	...
	public void generator() throws Exception {
		...
		String intfPath = generatePath(coral.getName(),daoIntfPackage);
		String implPath = generatePath(coral.getName(),daoImplPackage);
		...
		System.out.println("*** Go "+coral.getName() + " Spring tags***");
		for (Entity entity : entityList) {
			printoutSpringXML(entity);
			String classContent = generateDaoImplement(entity);
			generateFile(implPath, classContent,daoImplementName(entity.getEntityName()));
		}
		System.out.println("*** End "+coral.getName() + " Spring tags***");
		String classContent = generateDaoFactory();
		generateFile(implPath, classContent,daoFactory);
	}
	
	private String generatePath(String prefix, String classPath) {
		String path = null;
		if (classPath != null) {
			path = default_dao_path + "\\"+prefix+"\\" + classPath.replace(".", "\\")+ "\\";
		}
	...
	private void printoutSpringXML(Entity entity) {
		String entityName = entity.getEntityName();
		String implName = daoImplementName(entity.getEntityName());
		String daoVeriable = daoVariableName(entityName);
		System.out.println("<bean id=\""+daoVeriable+"\" class=\""+daoImplPackage+"."+ implName +"\"/>");
	}
...
//	    sb.append(tabTag2 +"return null" + endTag);
	...
	private String generateCreateMethod(Entity entity) {
		...
		sb.append(tabTag3 + "Connection con = db.getConnection();"+ spaceTag);
...
//				sb.append(tabTag3 + "ps."+ setter(props.get(i).getType())+"(pos++, DateUtils.toSqlDate(entity."+ getter(props.get(i).getPropertyName()) +"));"+ spaceTag);
	...
	public static void main(String[] args) {
	    try {
	      ...
	      DaoGenerater entityGenerator = new DaoGenerater(coralList.get(0));
